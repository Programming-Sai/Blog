...
const ProgressBar = ({ key, color, title, value, total, details }) => {
  const percentage = (parseFloat(value) / total) * 100; 
  return(
    <div key={key} className={styles.progressContainer}>
      <p>{ title }({ value } - { details })</p>
      <div className={styles.progressbar} style={{'--color': color, '--percentage': percentage}} />
    </div>
  )
};
        ...
        <div className={styles.progresses}>
          {
            serverMetrics.map((metrics, i)=>(
              <ProgressBar key={i}  color={ metrics.color } title={metrics.name} value={ metrics.value } total={ metrics.total } details={metrics.details} />
            ))
          }
        </div>
    </div>
